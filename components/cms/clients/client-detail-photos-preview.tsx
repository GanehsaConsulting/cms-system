...
interface ClientDetailPhotosPreviewProps {
  images: string[];
  title: string;
}

export function ClientDetailPhotosPreview({
  images,
  title,
}: ClientDetailPhotosPreviewProps) {
  ...
  return (
    <div className="space-y-2">
      <button
        type="button"
        aria-label="Preview client photo"
        onClick={() =>
          openPreview({
            images,
            index: 0,
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-0.5">
          {rest.slice(0, 4).map((image, index) => (
            <button
              key={image}
              type="button"
              aria-label={`Preview photo ${index + 2}`}
              onClick={() =>
                openPreview({
                  images,
                  index: index + 1,
