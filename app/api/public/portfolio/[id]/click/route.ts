...
import { incrementPortfolioClick } from "@/lib/db/portfolio";
...
export async function GET(
  ...
  try {
    const clickCount = await incrementPortfolioClick(
      result.brand.id,
      decodeURIComponent(id),
    );
    return publicJson({ id: decodeURIComponent(id), clickCount });
  } catch {
    return publicError("Portfolio item not found", 404);
  }
}
